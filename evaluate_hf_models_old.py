...
datasets_info = {
    "irmas": {
        "path": "/home/mlaquatra/audio_datasets/irmas/",
        "max_length_seconds": 5,
        "is_multilabel": True
    }
}

enabled_datasets = ["irmas"]
tsv_line = ""
...
model = Wav2Vec2ModelWrapper(audio_model, feature_extractor, args.device, max_length=30*16_000)
MAGNATAGATUNE_DATASET_PATH = "/data1/mlaquatra/datasets/audio_datasets/magnatagatune/"
evaluator_magnatagatune = MagnaTagATune(path=MAGNATAGATUNE_DATASET_PATH, verbose=args.verbose, is_top50=True)

res = []
for i in range(args.n_iters):
    if args.verbose:
        print(f"Iteration {i+1}/{args.n_iters}")
        print ("----------------- MagnaTagATune TOP-50 linear -----------------")

    res_magnatagatune = evaluator_magnatagatune.evaluate(model, mode="linear", device=args.device, batch_size=32, max_num_epochs=args.max_epochs)

    if args.verbose:
        print("Iteration: ", i+1)
        for metric, value in res_magnatagatune.items():
            print (f"{metric}: {value}")

    res.append(res_magnatagatune)
...
tsv_line = f"{args.model}\t{model_parameters}\tTrue\tMagnaTagATune-top50\t{res_mean['map_macro']}\t{res_std['map_macro']}\t{res_mean['map_weighted']}\t{res_std['map_weighted']}\n"

res = []
for i in range(args.n_iters):
    if args.verbose:
        print(f"Iteration {i+1}/{args.n_iters}")
        print ("----------------- MagnaTagATune TOP-50 non-linear -----------------")

    res_magnatagatune = evaluator_magnatagatune.evaluate(model, mode="non-linear", device=args.device, batch_size=32, max_num_epochs=args.max_epochs)
...
# create a tsv line: model_tag, size, is_linear, dataset_name, mean_map_macro, std_map_macro, mean_map_weighted, std_map_weighted
tsv_line += f"{args.model}\t{model_parameters}\tFalse\tMagnaTagATune-top50\t{res_mean['map_macro']}\t{res_std['map_macro']}\t{res_mean['map_weighted']}\t{res_std['map_weighted']}\n"
